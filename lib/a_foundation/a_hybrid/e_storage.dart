...
class Storage {
  ...
  static Future<String?> createURLByPath({
    ...
    if (FirebaseInitializer.isUsingOfficialPackages() == true){
      _url = await OfficialStorage.createURLByPath(
        path: path,
      );
    }

    else {
      _url = await NativeStorage.createURLByPath(
        path: path,
      );
    }
  ...
  static Future<Uint8List?> readBytesByURL({
    ...
    if (FirebaseInitializer.isUsingOfficialPackages() == true){
      _output = await OfficialStorage.readBytesByURL(
        url: url,
      );
    }

    else {
      _output = await NativeStorage.readBytesByURL(
        url: url,
      );
    }
  ...
  static Future<void> updateMetaByPath({
    ...
    if (FirebaseInitializer.isUsingOfficialPackages() == true) {
      await OfficialStorage.updateMetaByPath(
        path: path,
        meta: meta,
      );
    }
      ...
      await NativeStorage.updateMetaByPath(
        path: path,
        meta: meta,
      );
    }
  ...
  static Future<bool> move({
    ...
    if (FirebaseInitializer.isUsingOfficialPackages() == true) {
      _output = await OfficialStorage.move(
        oldPath: oldPath,
        newPath: newPath,
      ...
      _output = await NativeStorage.move(
        oldPath: oldPath,
        newPath: newPath,
        currentUserID: currentUserID
      );
  ...
  static Future<void> completeMeta({
    ...
    if (FirebaseInitializer.isUsingOfficialPackages() == true) {
      await OfficialStorage.completeMeta(
        path: path,
        currentUserID: currentUserID,
        addOwners: addOwners,
      );
      ...
      await NativeStorage.completeMeta(
        path: path,
        currentUserID: currentUserID,
        addOwners: addOwners,
      );
